...
class I18NPlugin(Plugin):
    ...
    def on_before_build(self, builder, build_state, source, prog, **extra):
        ...
        if (
            ...
            for language in self.translations_languages:
                ...
                translated_filename = join(
                    dirname(source.source_filename), "contents+%s.lr" % language
                )
                with codecs.open(contents, encoding="utf-8") as file:
    ...
    def __trans_linewise(self, content, translator):
        ...
        for line in content.split("\n"):
            ...
            trans_stripline = ""
            if line_stripped:
                trans_stripline = trans(
                    translator, line_stripped
    ...
    def __trans_parwise(self, content, translator):
        ...
        for paragraph in split_paragraphs(content):
            ...
            paragraph = paragraph.replace(stripped, trans(translator, stripped))
    ...
    def on_after_build(self, builder, build_state, source, prog, **extra):
        if self.enabled and isinstance(source, Page):
            try:
                with codecs.open(source.source_filename, encoding="utf-8") as f:
                    text = f.read()
            except IOError:
                pass
            else:
